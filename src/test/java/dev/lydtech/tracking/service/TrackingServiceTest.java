...
class TrackingServiceTest {
    ...
    @Test
    void sendStatus_CompletedSuccess() throws Exception {
    ...
    @Test
    void sendStatus_CompletedProducerThrowsException() throws Exception {
        DispatchCompleted dispatchCompleted = TestEventData.buildDispatchCompletedRandomData();

        doThrow(new RuntimeException("Service throws exception")).when(kafkaTemplateMock).send(anyString(), any(TrackingStatusUpdated.class));

        Exception exception = assertThrows(RuntimeException.class, () -> trackingService.sendStatus(dispatchCompleted));
